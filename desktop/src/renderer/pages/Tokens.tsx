/**
 * Tokens — standalone page for personal and team automation tokens.
 *
 * In Personal mode: shows personal tokens scoped to local hosts.
 * In Teams mode: shows team tokens scoped to the active team's hosts.
 */
...
export default function Tokens() {
  ...
  const { teams, loading: teamsLoading, notSignedIn } = useTeams();
  const activeTeam = teams.find((t) => t.id === activeTeamId) ?? teams[0] ?? null;
  const { hosts: teamHosts } = useTeamHosts(activeTeam?.id ?? '');
  ...
  const canManage = isTeamMode
    ? viewerRole === 'owner' || viewerRole === 'admin'
    : true;

  const availableHosts = isTeamMode ? teamHosts : personalHosts;
  ...
  const reload = useCallback(() => {
    ...
    if (isTeamMode && activeTeam) {
      window.sshthing
        .teamsTokensList(activeTeam.id)
        .then((result) => setTokens(result.tokens ?? []))
        .catch((err: unknown) => setError((err as Error).message ?? 'Failed to load team tokens'))
  ...
  useEffect(() => {
    if (!isTeamMode) {
      window.sshthing.listHosts().then((res) => setPersonalHosts(res.hosts)).catch(() => {
        setPersonalHosts([]);
      });
    }
  }, [isTeamMode]);
  ...
  const handleCreate = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      if (isTeamMode && activeTeam) {
        const result = await window.sshthing.teamsTokensCreate(
          activeTeam.id,
          tokenName.trim(),
          selectedHostIds,
        );
  ...
  }, [tokenName, selectedHostIds, reload, isTeamMode, activeTeam]);
  ...
  const handleRevoke = useCallback(async () => {
    ...
    try {
      if (isTeamMode && activeTeam) {
        await window.sshthing.teamsTokensRevoke(activeTeam.id, revokeTarget.id);
      } else {
  ...
  }, [revokeTarget, reload, isTeamMode, activeTeam]);

  const handleDeleteRevoked = useCallback(async (token: TokenSummary) => {
    try {
      if (isTeamMode && activeTeam) {
        await window.sshthing.teamsTokensDeleteRevoked(activeTeam.id, token.id);
      } else {
  ...
  }, [reload, isTeamMode, activeTeam]);

  // ── Loading / guards ──
  if (isTeamMode && teamsLoading) {
    return (
      <div className="page-scroll" style={{ width: '100%', padding: '24px 20px' }}>
        <SkeletonRows count={3} />
      </div>
    );
  }

  if (isTeamMode && notSignedIn) {
    return (
      <div className="page-scroll" style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
        <EmptyState
          title="Sign in to use Teams"
          description="Team tokens require a signed-in account."
  ...
  if (isTeamMode && !activeTeam) {
    return (
      <div className="page-scroll" style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
        <EmptyState title="No team selected" description="Select a team to manage its tokens." />
      </div>
    );
  }
  ...
  return (
    <div className="page-scroll" style={{ width: '100%', padding: '16px 20px' }}>
      {/* Header */}
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>
                    ...
                    {token.hostCount ? ` · ${token.hostCount} hosts` : ''}
                  </div>
                </div>
                <span
                  className="chip"
          ...
          <div className="field">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 200, overflowY: 'auto', border: '1px solid var(--line)', borderRadius: 4, padding: 6 }}>
              {availableHosts.length === 0 ? (
                <span style={{ fontSize: 12, color: 'var(--muted)', padding: 4 }}>No hosts available</span>
              ) : (
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 4, background: 'var(--paper-2)', border: '1px solid var(--line)' }}>
            <span className="chip" style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.06em' }}>success</span>
            <div style={{ flex: 1, fontSize: 12 }}><code style={{ fontFamily: 'var(--font-mono)' }}>deploy.sh --prod</code></div>
            <div style={{ fontSize: 11, color: 'var(--muted)' }}>2 days ago</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 4, background: 'var(--paper-2)', border: '1px solid var(--line)' }}>
            <span className="chip" style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.06em' }}>success</span>
            <div style={{ flex: 1, fontSize: 12 }}><code style={{ fontFamily: 'var(--font-mono)' }}>deploy.sh --staging</code></div>
